...
console.log(`TC: ${(ptx.match(/mma\.sync/g)||[]).length} mma.sync, shmem=${shmem}`);
...
const tcArgsBase = Number(ptr(tcArgs));
const tcKp = Buffer.alloc((np + 1) * 8);
for (let i = 0; i < np; i++) tcKp.writeBigUInt64LE(BigInt(tcArgsBase + i * 8), i * 8);
...
const dqAddr = dA_q4k.readBigUInt64LE(0);
const dqOutAddr = dA_dq.readBigUInt64LE(0);
const gx = Math.ceil(N / 32), gy = Math.ceil(M / 32);
const FLOP = 2n * BigInt(M) * BigInt(N) * BigInt(K);
const ITERS = 100;

// Verify dequant
deq.launch(dqAddr, dqOutAddr, NUM_BLOCKS);
cs.cuCtxSynchronize();
const hDQ = new Int8Array(DQ_SIZE);
cs.cuMemcpyDtoH_v2(hDQ, Number(dqOutAddr), BigInt(DQ_SIZE));
let dqErr = 0;
...
for (let i = 0; i < ITERS; i++) {
  ...
  times.push(gpuTimeUs(cs, ev));
}
const avg = times.reduce((a, b) => a + b, 0) / times.length;
const tops = Number(FLOP) / (avg / 1e6) / 1e12;
console.log(`\nQ4_K×Q8_1  ${M}×${K}×${N}: ${avg.toFixed(1)} µs → ${tops.toFixed(2)} TFLOPS`);
