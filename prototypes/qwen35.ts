...
const w = (W:Map<string,bigint>,l:number,n:string) => { const v = W.get(`model.language_model.layers.${l}.${n}`); if(!v) throw new Error(`missing weight: layers.${l}.${n}`); return v; };
...
function launch(k:any,grid:number[],block:number[],args:any[],label:string) {
  const rc = cuLaunch(k, [grid[0],grid[1],grid[2]] as [number,number,number], [block[0],block[1],block[2]] as [number,number,number], args);
  const _sync = cuSync(); if (_sync) console.log("  [ERR] " + label + " sync rc=" + _sync); return rc || _sync;
  if (rc) console.log(`  [WARN] ${label} rc=${rc}`);
  return rc || _sync;
}
...
const kQKV=mm(seq,QKVD,H), kZ=mm(seq,ZD,H), kA=mm(seq,16,H), kB=mm(seq,16,H);
const kOP=mm(seq,H,ZD), kQP=mm(seq,4096,H), kKP=mm(seq,512,H), kVP=mm(seq,512,H), kOAt=mm(seq,H,2048);
const kGP=mm(seq,INTER,H), kUP=mm(seq,INTER,H), kDP=mm(seq,H,INTER), kLM=mm(seq,VOCAB,H);
...
console.log(`compiled ${17} kernels in ${((performance.now()-t0)/1000).toFixed(1)}s`);
...
let x = cuAlloc(BigInt(seq*H*2)); cuHtoD(x, hEmb.buffer); 
console.log(`embedded: ${seq}×${H} bf16`);

// forward pass
console.log("forward pass...");
const fStart = performance.now();
const BLK1 = Math.ceil(seq*H/1024);
...
for (let layer=0; layer<NL; layer++) {
  ...
  launch(kRms, [seq,1,1], [128,1,1], [x, w(W,layer,"input_layernorm.weight"), normed], `rms1.L${layer}`);

  // Attention (placeholder: zero output for now)
  const attnOut = cuAlloc(BigInt(seq*H*4));  // f32 zeros
  // TODO: wire GDN (linear) or FA2 (full) here
  if (layer < 4) console.log(`  layer ${layer} (${full?"full":"linear"}): attn placeholder`);

  // cast f32→bf16
  const attnBf16 = cuAlloc(BigInt(seq*H*2));
  launch(kCs, [BLK1,1,1], [128,1,1], [attnOut, attnBf16], `cast1.L${layer}`);
  // residual add
  const afterAttn = cuAlloc(BigInt(seq*H*2));
  launch(kAd, [BLK1,1,1], [128,1,1], [x, attnBf16, afterAttn], `add1.L${layer}`);
  cuFree(x); cuFree(attnOut); cuFree(attnBf16); cuFree(normed);

  // RMSNorm 2
  const normed2 = cuAlloc(BigInt(seq*H*2));
  launch(kRms, [seq,1,1], [128,1,1], [afterAttn, w(W,layer,"post_attention_layernorm.weight"), normed2], `rms2.L${layer}`);

  // MLP: gate, up, swiglu, down
  const gate = cuAlloc(BigInt(seq*INTER*4));
  launch(kGP, [Math.ceil(seq/64),Math.ceil(INTER/64),1], [128,1,1], [normed2, w(W,layer,"mlp.gate_proj.weight"), gate], `gate.L${layer}`);
  const up = cuAlloc(BigInt(seq*INTER*4));
  launch(kUP, [Math.ceil(seq/64),Math.ceil(INTER/64),1], [128,1,1], [normed2, w(W,layer,"mlp.up_proj.weight"), up], `up.L${layer}`);
  const act = cuAlloc(BigInt(seq*INTER*2));
  launch(kSg, [Math.ceil(seq*INTER/1024),1,1], [128,1,1], [gate, up, act], `swiglu.L${layer}`);
  cuFree(gate); cuFree(up); cuFree(normed2);
  const mlpOut = cuAlloc(BigInt(seq*H*4));
  launch(kDP, [Math.ceil(seq/64),Math.ceil(H/64),1], [128,1,1], [act, w(W,layer,"mlp.down_proj.weight"), mlpOut], `down.L${layer}`);
  cuFree(act);

  // cast + residual
  const mlpBf16 = cuAlloc(BigInt(seq*H*2));
  launch(kCs, [BLK1,1,1], [128,1,1], [mlpOut, mlpBf16], `cast2.L${layer}`);
  x = cuAlloc(BigInt(seq*H*2));
  launch(kAd, [BLK1,1,1], [128,1,1], [afterAttn, mlpBf16, x], `add2.L${layer}`);
  cuFree(afterAttn); cuFree(mlpOut); cuFree(mlpBf16);
}

// final norm + lm_head
const fn = cuAlloc(BigInt(seq*H*2));
launch(kRms, [seq,1,1], [128,1,1], [x, W.get("model.language_model.norm.weight")!, fn], `rmsFinal`);
...
console.log(`\nforward: ${dt.toFixed(2)}s (${NL} layers)`);
